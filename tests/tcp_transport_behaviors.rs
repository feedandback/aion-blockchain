...
async fn stream_after_client_writes(bytes: &[u8]) -> TcpStream {
    ...
    client.await.expect("test client task must complete");
...
async fn truncated_and_malformed_frames_remain_errors() {
    let mut partial_header_stream = stream_after_client_writes(&[0, 0]).await;
    let partial_header = TcpTransport::read_message_or_eof(&mut partial_header_stream).await;
    assert!(
        partial_header
            .expect_err("partial header must be rejected")
            .contains("header could not be read")
    );

    let mut partial_payload_frame = 8u32.to_be_bytes().to_vec();
    partial_payload_frame.extend_from_slice(b"{}");
    let mut partial_payload_stream = stream_after_client_writes(&partial_payload_frame).await;
    let partial_payload = TcpTransport::read_message_or_eof(&mut partial_payload_stream).await;
    assert!(
        partial_payload
            .expect_err("partial payload must be rejected")
            .contains("message could not be read")
    );

    let malformed_payload = b"not-json";
    let mut malformed_frame = u32::try_from(malformed_payload.len())
        .expect("test payload length must fit u32")
        .to_be_bytes()
        .to_vec();
    malformed_frame.extend_from_slice(malformed_payload);
    let mut malformed_stream = stream_after_client_writes(&malformed_frame).await;
    let malformed = TcpTransport::read_message_or_eof(&mut malformed_stream).await;
    assert!(
        malformed
            .expect_err("malformed JSON must be rejected")
            .contains("JSON format is invalid")
    );
}
...
async fn valid_frame_is_read_before_the_clean_session_close() {
    ...
    let payload = serde_json::to_vec(&message).expect("test message must serialize");
...
async fn authenticated_client_message_advertises_the_one_shot_address() {
    ...
    let server = tokio::spawn(async move {
        ...
        let (mut stream, _, handshake) =
            TcpTransport::accept_authenticated(&listener, &server_wallet)
                .await
    ...
    let (handshake, message) = server.await.expect("test server task must complete");
...
async fn authenticated_request_receives_transaction_ack_on_the_same_connection() {
    ...
    let handshake = server.await.expect("test server task must complete");
